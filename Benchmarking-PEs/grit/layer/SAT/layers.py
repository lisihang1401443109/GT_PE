...
class StructureExtractor(nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr=None,
        ...
        if self.khopgnn:
            if agg == "sum":
                x = scatter_add(x, subgraph_indicator_index.to(dtype=torch.int64, device=x.device), dim=0)
            elif agg == "mean":
                x = scatter_mean(x, subgraph_indicator_index.to(dtype=torch.int64, device=x.device), dim=0)
            return x
