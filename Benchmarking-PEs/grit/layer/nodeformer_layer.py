...
class NodeFormer(nn.Module):
    ...
    '''
    def __init__(self, in_dim, hid_dim, out_dim, num_layers=2, num_heads=4, dropout=0.0,
        ...
        self.fcs.append(nn.Linear(self.in_dim, self.hid_dim))
        self.bns = nn.ModuleList()
        self.bns.append(nn.LayerNorm(self.hid_dim))
        for i in range(self.num_layers):
            self.convs.append(
                NodeFormerConv(self.hid_dim, self.hid_dim, num_heads=self.num_heads, kernel_transformation=kernel_transformation,
        ...
        if use_jk:
            self.fcs.append(nn.Linear(self.hid_dim * self.num_layers + self.hid_dim, self.out_dim))
        else:
            self.fcs.append(nn.Linear(self.hid_dim, self.out_dim))
    ...
    def forward(self, batch, tau=1.0):
        batch = batch[0]
        x = batch.x.unsqueeze(0) # [B, N, H, D], B=1 denotes number of graph
        layer_ = []
        link_loss_ = []
        z = self.fcs[0](x)
        if self.use_bn:
