...
def parse_run(name):
    ...
    elif name.startswith("IMDB-Ablation-"):
        parts = name.split("-")
        # IMDB-Ablation-Sparse-GPSE-Final
        if len(parts) < 4:
            return None
        pe = parts[3]
        model = parts[2]
    ...
    if pe not in PE_COLORS:
        # Check if it's lowercase or has other suffixes
        for k in PE_COLORS:
            if k.lower() == pe.lower():
                pe = k
                break
    ...
    if model not in MODEL_STYLES:
        if "Sparse" in model or "sparse" in model.lower():
            model = "Sparse"
        elif "Dense" in model or "dense" in model.lower():
            model = "Dense"
        elif "GAT" in model or "gat" in model.lower():
            model = "GAT"
        else:
            return None
